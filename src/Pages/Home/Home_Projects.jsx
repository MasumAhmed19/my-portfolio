    ...
    >
      ...
      >
        <div className="text-xs flex items-center gap-1">
           Detail <FiArrowRight className=" -rotate-45 text-secondary-dark" />
        </div>
      ...
      <section className="p-4 md:p-8">
        <div className="mx-auto max-w-4xl">
          {projects.map((project, index) => (
            <LinkProject key={index} {...project} onClick={() => setModalData(project)} />
          ))}
        </div>
  
        {modalData && <SpringModal isOpen={!!modalData} setIsOpen={() => setModalData(null)} modalData={modalData} />}
      </section>
    );
  };
